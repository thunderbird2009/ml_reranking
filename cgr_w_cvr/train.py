"""Training loop for the CVR-extended CGR variant.

The training objective is a weighted sum of three binary cross-entropy losses:

    L = λ_exp · L_exp  +  λ_clk · L_clk  +  λ_cvr · L_cvr
...
* L_cvr = BCE(p_cvr, y_cvr) — conversion prediction loss.  y_cvr is 1 when
    the item converted.
...
class TrainConfig:
    ...
    Attributes:
        ...
        lambda_exp: weight for the exposure BCE loss.
          ...
          lambda_cvr: weight for the conversion BCE loss.
        click_positive_class_weight: positive-class weight for the click loss.
          ...
          cvr_positive_class_weight: positive-class weight for the conversion loss.
              This is useful because conversion labels are usually even sparser
              than click labels.
    ...
    cvr_positive_class_weight: float = 1.0
    epochs: int = 10
    batch_size: int = 32
    grad_clip: float = 1.0
...
class CGRTrainer:
    ...
    def __init__(self, model: CGRModel, config: TrainConfig, device: torch.device):
        ...
        self.loss_cvr = nn.BCEWithLogitsLoss(
            pos_weight=torch.tensor(config.cvr_positive_class_weight, device=device)
        )
    ...
    def train_step(
        ...
        The forward pass produces per-position logits for exposure, click, and conversion.
        These are compared against the logged binary labels via
        ``BCEWithLogitsLoss``, weighted by λ_exp, λ_clk, and λ_cvr, and the combined
        ...
        Args:
            ...
            cvr_labels: [B, L] binary ground-truth conversion labels (from logs).

        Returns:
            dict with keys ``"loss"``, ``"loss_exp"``, ``"loss_clk"``, ``"loss_cvr"``.
        """
        self.model.train()
        self.optimizer.zero_grad()

        exp_logits, clk_logits, cvr_logits = self.model.forward_logits(
        ...
        cvr_labels = cvr_labels.to(self.device)

        # Multi-task loss over exposure, click, and conversion.
        loss_exp = self.loss_exp(exp_logits, exp_labels)
        loss_clk = self.loss_clk(clk_logits, clk_labels)
        loss_cvr = self.loss_cvr(cvr_logits, cvr_labels)
        loss = (
            self.config.lambda_exp * loss_exp
            + self.config.lambda_clk * loss_clk
            + self.config.lambda_cvr * loss_cvr
        )
    ...
    def train_epoch(self, dataloader: torch.utils.data.DataLoader) -> dict[str, float]:
        ...
        Expects each batch from the dataloader to be a dict with keys::
            ...
            item_embs, user_embs, context_embs, item_types, exp_labels, clk_labels, cvr_labels

        Returns:
            dict with epoch-averaged ``"loss"``, ``"loss_exp"``, ``"loss_clk"``, ``"loss_cvr"``.
        """
        total_loss = 0.0
        total_exp = 0.0
        total_clk = 0.0
        ...
        return {
            ...
            "loss_cvr": total_cvr / max(n_batches, 1),
        }
