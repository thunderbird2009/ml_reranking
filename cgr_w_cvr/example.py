"""End-to-end CVR-extended CGR example with synthetic data.

Demonstrates:
1. Simulating pre-computed upstream embeddings (as would come from a DLRM pipeline)
2. Training the model on synthetic exposure/click/conversion labels
3. Running two-stage constrained inference with per-item profit margin in reward
...
def make_synthetic_dataset(
    ...
    the upstream pipeline alongside logged exposure/click/conversion labels.
    """
    ...
    # Logged labels: exposure ~ 0.7 base, click ~ 0.3 base, conversion is a
    # sparse subset of clicks.
    exp_labels = (torch.rand(n_samples, list_len) > 0.3).float()
    clk_labels = (torch.rand(n_samples, list_len) > 0.7).float() * exp_labels
    cvr_labels = (torch.rand(n_samples, list_len) > 0.9).float() * clk_labels
...
def collate_fn(batch):
    item_e, user_e, ctx_e, types, exp, clk, cvr = zip(*batch)
    return {
...
def run():
    ...
    print("\n--- Training ---")
    config = TrainConfig(lr=1e-3, epochs=5, batch_size=32, lambda_cvr=1.0, cvr_positive_class_weight=3.0)
    ...
    organic_items = [
        make_synthetic_item(
            i,
            ItemType.ORGANIC,
            engagement=1.0 + 0.1 * i,
            profit_margin=0.8 + 0.15 * i,
        )
        for i in range(8)
    ]
    ad_candidates = [
        make_synthetic_item(100, ItemType.AD, bid=2.5, engagement=0.5, ad_penalty=0.3, profit_margin=0.2),
        make_synthetic_item(101, ItemType.AD, bid=3.0, engagement=0.3, ad_penalty=0.4, profit_margin=0.25),
        make_synthetic_item(102, ItemType.LARGE_AD, bid=5.0, engagement=0.8, ad_penalty=0.5, profit_margin=0.35),
        make_synthetic_item(103, ItemType.ECOM_AD, bid=1.5, engagement=0.6, ad_penalty=0.2, profit_margin=0.18),
    ]
    ...
    for i, item in enumerate(result.items):
        ...
        margin_str = f"  margin={item.profit_margin:.2f}"
        print(f"    [{i}] item_{item.item_id} ({item.item_type.name}){bid_str}{margin_str}{marker}")
