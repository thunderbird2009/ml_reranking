...
The code follows the paper's overall decomposition:
...
- [model.py](model.py): `CGRModel`, `HierarchicalAttentionBlock`, `PLEFusion`, and `ExposureClickHead`
- [train.py](train.py): `TrainConfig` and `CGRTrainer` for weighted multi-task training
...
Those expert outputs are fused by `PLEFusion`, and `ExposureClickHead` produces `p_exp` and `p_clk`.
